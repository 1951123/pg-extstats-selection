...
{\footnotesize candidate discovery = discovery of correlated column groups; multivariate statistics = multi-column statistics; E2E = end-to-end planner validation.}
\end{table*}
...
{\footnotesize $b$ = sub-batch number; $s$ = candidates in the sub-batch; total/ANALYZE/mask are seconds; $nL$ = distinct (candidate$\times$level) objects measured.}
\end{table*}
...
\begin{table*}[t]
\caption{Workload characteristics and candidate distribution. ``cands/query'' =
sum of per-query candidate counts before dedup; ``global dedup'' = distinct
column combinations; ``per-query'' = min/p25/med/p75/max of candidates/query.}
\label{tab:bench}
\small
...
{\footnotesize cands/query = total per-query candidate count before dedup; global dedup = distinct column combinations; per-query = min/p25/med/p75/max candidates per query.}
\end{table*}
...
{\footnotesize All columns are q-error values; $\operatorname{cov}_K$ = fraction of the best $K$-statistic improvement captured by the single best statistic.}
\end{table}
...
{\footnotesize Each q-error entry is the workload mean; $\Delta$q = MILP advantage over the best greedy; best-is = the winning greedy at that budget.}
\end{table}
...
{\footnotesize predicted = model-predicted q-error; E2E = q-error measured on the real table; ratio = E2E/predicted.}
\end{table*}
...
{\footnotesize mean = achieved workload-mean q-error; tail = queries with E2E/predicted ratio $>1.05$ (\emph{of served}, out of the total served per budget).}
\end{table}
