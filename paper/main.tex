...
1000\,KB  & 0.96 & 39  & 2.3 & 457\\
\bottomrule
\end{tabular}

{\footnotesize Same machine as \S\ref{sec:setup}.}
\end{table}
...
The same pattern reproduces on \textsc{stats-CEB-single}. At its 100\,KB budget the
uncapped solution tops out at L10000 (estimated \texttt{ANALYZE} $\approx\!31$\,s),
yet capping the maximum target to L1000 leaves both storage and geometric-mean
q-error \textit{exactly unchanged} ($99{,}996$\,B; $1.0156$) while cutting the
estimated \texttt{ANALYZE} rebuild to $\approx\!12$\,s --- a $2.7\times$ reduction
in maintenance cost at zero statistical cost. At the two tighter budgets (10\,KB,
40\,KB) the capped and uncapped solutions are byte- and value-identical (maximum-target
delta of 0), confirming that a tight budget simply never has room to host a high-target
statistic that matters. As on \textsc{Census}, the mismatch is confined to the
generous-budget regime.
...
To quantify this degradation, we express the expected sample count as
$\lambda=p_{\mathrm{comb}}\cdot\mathrm{sample\_rows}(L)$ and measure the
fidelity ratio $\rho=\mathrm{qerr}_{\mathrm{true}}/\mathrm{qerr}_{\mathrm{mask}}$
(the real deployed q-error over the Catalog-Mask oracle) across a sweep of
$\lambda$. Table~\ref{tab:fidelity-lambda} reports this sweep on \textsc{Census}
for a rare driver combination whose true cardinality is $13$ among $2.46$M rows
($p_{\mathrm{comb}}\approx5\times10^{-6}$). The transition is sharp and
systematic: at $\lambda=0.16$ the real q-error is \emph{$224\times$} the oracle
prediction; the gap shrinks monotonically as $\lambda$ grows and closes entirely
by $\lambda\approx5$ ($\rho=1.1$) --- for $\lambda\ge10$ the real deployment
reaches the oracle exactly. The same divergence-to-convergence shape reproduces
across all three candidate statistics we tested, confirming it is a mechanistic
property of the combination-to-sampling path rather than a single-query
artifact.

Crucially, the $\lambda$ threshold cleanly separates the two query populations
we seek to distinguish. For \emph{high-occurrence} combinations, $\lambda$ is
abundant and fidelity is robust at every practical target: on
\textsc{stats-CEB-single}, single-statistically deploying a frequent
\texttt{posts} combination (AnswerCount, FavoriteCount) yields $\rho=1.0000$ for
all 15 queries at every target from L25 to L10000 (75 points, zero gaps). In
contrast, \emph{rare} combinations ($\lambda\le1$) are precisely the ones
prone to oracle over-optimism: their true q-error can be orders of magnitude
above the Catalog-Mask prediction. $\lambda$ therefore serves as a valid,
cheaply computable warning heuristic: deployments with abundant $\lambda$ inherit
the oracle's accuracy, while combinations with $\lambda\le1$ should trigger
a fidelity check or a larger target before their predicted benefit is trusted.

\begin{table}[t]
\caption{Fidelity transition on \textsc{Census}: real q-error / oracle q-error
($\rho$) vs.\ expected sample count $\lambda$ for a rare driver combination
(true cardinality $13$ / $2.46$M rows).}
\label{tab:fidelity-lambda}
\small
\begin{tabular}{cccc}
\toprule
$\lambda$ & $L$ & real qerr & $\rho$\\
\midrule
0.16 & 30---100 & 117---242 & 109---224\\
0.32 & 200       & 24          & 22\\
0.63 & 400       & 22          & 20\\
1.27 & 800       & 11          & 10\\
2.54 & 1600      & 2.8         & 2.6\\
...
{\footnotesize $\lambda=p_{\mathrm{comb}}\cdot\mathrm{sample\_rows}(L)$;
$\rho=1$ means the real deployment matches the Catalog-Mask oracle exactly.}
\end{table}
...
\paragraph{Q4: Query-internal cluster decomposition for sufficiency-violating workloads.}
One potential extension for workloads violating one-stat sufficiency is
\emph{query-internal cluster decomposition}: decompose a query containing
multiple independent correlation clusters into several sub-problems, where each
sub-problem corresponds to one individual correlated cluster and obeys one-stat
sufficiency. We could optimize candidate statistics for each decomposed cluster
separately, and combine the resulting candidates under the global storage
budget. Nevertheless, this extension introduces non-trivial new challenges:
automatic detection of independent correlation clusters from the
schema-workload is non-trivial; merging per-cluster candidates may
re-introduce overlapping statistics and planner interference; furthermore,
local per-cluster optimality does not guarantee global optimality under a
shared storage budget. For these reasons we leave this end-to-end
decomposition-and-merge pipeline as future work.
